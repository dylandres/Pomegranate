...
function QuizTaking() {
    ...
    const [stopwatch2, setStopwatch2] = useState(0);

    useEffect(() => {
        // Stopwatch mechanism
            setTimeout(() => setStopwatch2(stopwatch2 + 1), 1000);
    });
    ...
    const fillQuestions = async (quizName) => {
        ...
        if (thisQuiz[0].questions != null) {
            var questionArray = []
            var correctArray = []
            for (var i = 0; i < thisQuiz[0].questions.length; i++) {
                const question = thisQuiz[0].questions[i];
            ...
            <br />
            {
                userObject ? 
                                    ...
                                    <ul class="question">
                                        ...
                                        <div class="question-text">
                                            <div class='question-inner'>
                                                ...
                                                <img style = {{width: '25%'}} className="question-image" src={questions[questionIndex].image}></img> : null}
                                            </div>
                                        </div>
                    ...
                    : stopwatch2 < 2 ? 
                        <div className='loader'></div>
                        :
                        <div style={{position: 'absolute', transform: 'translateX(-50%) translateY(-50%)', left: '50%', top: '50%'}}>You are not logged in!</div>
                }
        </body>
    );
}
